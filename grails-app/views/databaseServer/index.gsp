...
<body>
<a href="#list-databaseServer" class="skip" tabindex="-1"><g:message code="default.link.skip.label"
...
<div id="list-databaseServer" class="content scaffold-list" role="main">
  ...
  <table>
    ...
    <tr>
      ...
      <g:sortableColumn property="ipAddress"
                        title="${message(code: 'databaseServer.ipAddress.label', default: 'IP Address')}"/>
      ...
      <g:sortableColumn property="identity"
                        title="${message(code: 'databaseServer.identity.label', default: 'Identity')}"/>

      <g:sortableColumn property="instanceId"
                        title="${message(code: 'databaseServer.instanceId.label', default: 'Instance Id')}"/>

      <g:sortableColumn property="loginUser"
                        title="${message(code: 'databaseServer.loginUser.label', default: 'Login User')}"/>
    ...
    <g:each in="${databaseServerInstanceList}" status="i" var="databaseServerInstance">
      <tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
        ...
        <td><g:link action="show"
                    id="${databaseServerInstance.id}">${fieldValue(bean: databaseServerInstance, field: "ipAddress")}</g:link></td>

        <td>${fieldValue(bean: databaseServerInstance, field: "host")}</td>

        <td>${fieldValue(bean: databaseServerInstance, field: "identity")}</td>

        <td>${fieldValue(bean: databaseServerInstance, field: "instanceId")}</td>
  ...
  <div class="pagination">
    <g:paginate total="${databaseServerInstanceCount ?: 0}"/>
  </div>
</div>
</body>
</html>
